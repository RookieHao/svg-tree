...
const RECURSIVE_DATA = Symbol('RECURSIVE_DATA')
...
class Tree extends customEvent {
  ...
  [DRAW_NODE] (group, data, startX = 0, parent_middleY) {
    ...
    data.forEach((item, index, data) => {
      ...
      const { height: text_height } = node_title.getBBox()
      // icon_size
      ...
      node_title.setAttribute('dy', text_height - 4)

      const {
        width: text_width,
        y: text_Y,
        x: text_X } = node_title.getBBox()

      // draw node_rect
      let node_rect = drawRect({ x: text_X - 5, y: this[NODE_TOP], width: text_width + 10, height: text_height, rx: 1, ry: 1 })
      rect_g.insertBefore(node_rect, node_title)

      // bind click
      rect_g.addEventListener('click', () => {
        if (rect_g.className.animVal.indexOf('active-node') < 0) {

          let allActiveNode = document.querySelectorAll('.active-node')

          for (let i = 0, len = allActiveNode.length; i < len; i++) {
            allActiveNode[i].setAttribute('class', allActiveNode[i].className.animVal.replace('active-node', '').trim())
          }
      ...
      let NODE_VIEW_WIDTH = text_X + text_width + 5
  ...
  [RECURSIVE_DATA] (data, expand) {
    const { child_field } = this.parameter
    data.forEach(d => {
      d = Object.assign(d, {
        [child_field]: (expand ? d[child_field] || d['__old_' + child_field] : null),
        ['__old_' + child_field]: (expand ? null : d[child_field] || d['__old_' + child_field]),
      })
      if(d[child_field] || d['__old_' + child_field]){
        this[RECURSIVE_DATA](d[child_field] || d['__old_' + child_field],expand)
      }
    })
  }

  expandAll () {
    this[RECURSIVE_DATA](this.tData, true)
    this.Init(this.tData)
  }

  collapseAll () {
    this[RECURSIVE_DATA](this.tData, false)
    this.Init(this.tData)
  }
}
